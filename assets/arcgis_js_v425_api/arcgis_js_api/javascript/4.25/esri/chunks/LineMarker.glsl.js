...
function(q,l,r,f,x,y,z,A,B,C,D,E,t,F,u,v,c,G,H,I,J,m,n){function w(a){const b=new H.ShaderBuilder,p=a.hasMultipassTerrain&&(a.output===f.ShaderOutput.Color||a.output===f.ShaderOutput.Alpha),g=a.space===n.LineMarkerSpace.World;a.hasTip&&g&&b.extensions.add("GL_OES_standard_derivatives");b.include(y.RibbonVertexPosition,a);b.include(A.MarkerSizing,a);a.output===f.ShaderOutput.Depth&&b.include(z.OutputDepth,a);const {vertex:d,fragment:k}=b;k.include(E.RgbaFloatEncoding);t.addProjViewLocalOrigin(d,a);
b.attributes.add(m.VertexAttribute.POSITION,"vec3");b.attributes.add(m.VertexAttribute.UV0,"vec2");b.attributes.add(m.VertexAttribute.AUXPOS1,"vec3");b.varyings.add("vColor","vec4");b.varyings.add("vpos","vec3");b.varyings.add("vUV","vec2");b.varyings.add("vSize","float");r.addLinearDepth(b);p&&b.varyings.add("depth","float");a.hasTip&&b.varyings.add("vLineWidth","float");d.uniforms.add([new F.Float2PassUniform("nearFar",(h,e)=>e.camera.nearFar),new u.Float4PassUniform("viewport",(h,e)=>e.camera.fullViewport)]);
...
return vec2(v.y, -v.x);
    ...
    `);a.space===n.LineMarkerSpace.Screen&&(d.uniforms.add(new G.Matrix4PassUniform("inverseProjectionMatrix",(h,e)=>e.camera.inverseProjectionMatrix)),d.code.add(c.glsl`vec3 inverseProject(vec4 posScreen) {
...
return abs(cos) > 0.001 && t > 0.0;
}`),d.uniforms.add(new v.FloatPassUniform("perScreenPixelRatio",(h,e)=>e.camera.perScreenPixelRatio)),d.code.add(c.glsl`
  ...
  `));d.uniforms.add(new v.FloatPassUniform("pixelRatio",(h,e)=>e.camera.pixelRatio));r.addCalculateLinearDepth(b);d.code.add(c.glsl`void main(void) {
if (uv0.y == 0.0) {
...
return clamp(0.5 - distance, 0.0, 1.0);
  ...
  void main() {
    ...
    ${a.output===f.ShaderOutput.Alpha?c.glsl`gl_FragColor = vec4(finalColor.a);`:""}
    ${a.output===f.ShaderOutput.Color?c.glsl`gl_FragColor = highlightSlice(finalColor, vpos);`:""}
    ${a.output===f.ShaderOutput.Color&&a.transparencyPassType===J.TransparencyPassType.Color?"gl_FragColor \x3d premultiplyAlpha(gl_FragColor);":""}
    ${a.output===f.ShaderOutput.Highlight?c.glsl`gl_FragColor = vec4(1.0);`:""}
    ${a.output===f.ShaderOutput.Depth?c.glsl`outputDepth(linearDepth);`:""}
  }
