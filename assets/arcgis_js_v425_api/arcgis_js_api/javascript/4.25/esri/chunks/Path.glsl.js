...
f=>f.opacity)]);c.include(D.ColorConversion);A.addMainLightIntensity(c);c.code.add(e.glsl`
        void main() {
          ...
          ${a.transparencyPassType===F.TransparencyPassType.Color?"gl_FragColor \x3d premultiplyAlpha(gl_FragColor);":""}
        }
      `);break;case d.ShaderOutput.Depth:case d.ShaderOutput.Shadow:case d.ShaderOutput.ShadowHighlight:case d.ShaderOutput.ShadowExludeHighlight:b.include(k.Transform,a);q.addNearFar(b);b.varyings.add("depth","float");g.code.add(e.glsl`void main() {
vpos = calculateVPos();
