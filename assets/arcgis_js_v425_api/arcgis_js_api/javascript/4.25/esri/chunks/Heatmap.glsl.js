...
b.include(l.DiscardOrAdjustAlphaBlend);({usesHalfFloat:c}=c);b.fragment.uniforms.add([new g.Texture2DPassUniform("densityMap",a=>a.densityMap),new g.Texture2DPassUniform("tex",a=>a.colorRamp),new d.FloatPassUniform("densityNormalizer",a=>1/(a.maxDensity-a.minDensity)),new d.FloatPassUniform("minDensity",a=>a.minDensity)]);b.fragment.uniforms.add(new d.FloatPassUniform("densityMultiplier",a=>3/(a.searchRadius*a.searchRadius*Math.PI)));c&&b.constants.add("compressionFactor","float",4);b.fragment.code.add(f.glsl`
    void main() {
      float density = texture2D(densityMap, uv).r * densityMultiplier${c?f.glsl` * compressionFactor`:""};
