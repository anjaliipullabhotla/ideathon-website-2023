...
c.include(B.Offset),a.instancedColor&&c.attributes.add(S.VertexAttribute.INSTANCECOLOR,"vec4"),h.add("vNormalWorld","vec3"),h.add("localvpos","vec3"),a.hasMultipassTerrain&&h.add("depth","float"),c.include(H.TextureCoordinateAttribute,a),c.include(A.ForwardLinearDepth,a),c.include(G.SymbolColor,a),c.include(I.VertexColor,a),g.uniforms.add(new P.Float4PassUniform("externalColor",d=>d.externalColor)),h.add("vcolorExt","vec4"),g.code.add(b.glsl`
        void main(void) {
          ...
          ${a.hasMultipassTerrain?b.glsl`depth = (view * vec4(vpos, 1.0)).z;`:""}
          forwardLinearDepth();
      ...
      void main() {
        discardBySlice(vpos);
        ${a.hasMultipassTerrain?b.glsl`terrainDepthTest(gl_FragCoord, depth);`:""}
        ${a.hasColorTexture?b.glsl`
                vec4 texColor = texture2D(tex, ${a.hasColorTextureTransform?b.glsl`colorUV`:b.glsl`vuv0`});
...
d=>d.ambient),new x.Float3PassUniform("diffuse",d=>d.diffuse),new l.FloatPassUniform("opacity",d=>d.opacity),new l.FloatPassUniform("layerOpacity",d=>d.layerOpacity)]),a.hasColorTexture&&e.uniforms.add(new y.Texture2DPassUniform("tex",d=>d.texture)),c.include(f.PhysicallyBasedRenderingParameters,a),c.include(M.PhysicallyBasedRendering,a),e.include(w.MixExternalColor),c.extensions.add("GL_OES_standard_derivatives"),r.addMainLightIntensity(e),e.code.add(b.glsl`
      void main() {
        ...
        ${a.transparencyPassType===R.TransparencyPassType.Color?b.glsl`gl_FragColor = premultiplyAlpha(gl_FragColor);`:b.glsl``}
      }
