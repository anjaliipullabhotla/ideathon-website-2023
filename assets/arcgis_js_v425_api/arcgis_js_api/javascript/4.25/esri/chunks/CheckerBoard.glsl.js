...
function(e,m,n,p,q,f,g,r,t,h){function k(b){const a=new r.ShaderBuilder;a.extensions.add("GL_OES_standard_derivatives");const {vertex:l,fragment:c}=a;p.addProjViewLocalOrigin(l,b);a.attributes.add(h.VertexAttribute.POSITION,"vec3");a.attributes.add(h.VertexAttribute.UV0,"vec2");a.varyings.add("vUV","vec2");b.hasMultipassTerrain&&a.varyings.add("depth","float");l.code.add(g.glsl`
    void main(void) {
      vUV = uv0;
  ...
  `);a.include(m.multipassTerrainTest,b);c.uniforms.add(new q.Float2PassUniform("size",d=>d.size));c.uniforms.add(new f.Float4PassUniform("color1",d=>d.color1));c.uniforms.add(new f.Float4PassUniform("color2",d=>d.color2));c.include(n.ColorConversion);c.code.add(g.glsl`
    void main() {
      ...
      ${b.transparencyPassType===t.TransparencyPassType.Color?"gl_FragColor \x3d premultiplyAlpha(gl_FragColor);":""}
    }
